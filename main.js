...
function createTree() {
  ...
  loader.load(
    ...
    function (gltf) {
      ...
      gltf.scene.position.set(Math.floor(Math.random()*(240+1)),15,Math.floor(Math.random()*(240+1)));
...
function loadNPC() {
  ...
  loader.load(
    ...
    function (gltf) {
      gltf.scene.scale.set(5,5,5);
      gltf.scene.position.set(1,4,0);
      gltf.scene.onclick=function(){
        alert("charater clicked");
      }
      const c = gltf.scene;
      ...
      let localInertia = new Ammo.btVector3(0, 0, 0);
      let verticesPos = c.geometry.getAttribute("position"),
        array;
      let triangles = [];
      for (let i = 0; i < verticesPos.length; i += 3) {
        triangle.push({
